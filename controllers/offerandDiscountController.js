...
exports.createOffer = async (req, res) => {
  try {
    ...
    if (new Date(validFrom) >= new Date(validTill)) {
      return res
        .status(400)
        .json({ error: "validFrom must be before validTill." });
    }
    ...
    if (["flat", "percentage"].includes(type) && !discountValue) {
      return res
        .status(400)
        .json({
          error: "discountValue is required for flat or percentage offer.",
        });
    }
    ...
    switch (type) {
      case "percentage":
        if (!maxDiscount) {
          return res
            .status(400)
            .json({ error: "maxDiscount is required for percentage offer." });
        }
        break;

      case "combo":
        if (!Array.isArray(comboProducts) || comboProducts.length < 1) {
          return res
            .status(400)
            .json({
              error: "At least 1 comboProduct is required for combo offer.",
            });
        }

        // Optional: validate each combo item structure
        for (const combo of comboProducts) {
          if (
            !combo.products ||
            !Array.isArray(combo.products) ||
            combo.products.length === 0 ||
            typeof combo.comboPrice !== "number"
          ) {
            return res
              .status(400)
              .json({
                error: "Each comboProduct must have products and a comboPrice.",
              });
          }
        }
      ...
      case "bogo":
        if (
          ...
          return res
            .status(400)
            .json({
              error: "Complete bogoDetails are required for BOGO offer.",
            });
        }
        break;
    }
    ...
    const offerData = {
      ...
      description,
      type,
      discountValue: ["flat", "percentage"].includes(type)
        ? discountValue
        : undefined,
      ...
      createdBy,
      createdByRestaurant:
        createdBy === "restaurant" ? createdByRestaurant : null,
      usageLimitPerUser,
      totalUsageLimit,
